...
kas = kas(find(kas < 2.5)); 
A_real = textread('A_real.txt');
A_real = A_real(1:length(kas));
A_imag = textread('A_imag.txt');
A_imag = A_imag(1:length(kas));
A_abs = textread('A_abs.txt');
A_abs = A_abs(1:length(kas));
B_real = textread('B_real.txt');
B_real = B_real(1:length(kas));
B_imag = textread('B_imag.txt');
B_imag = B_imag(1:length(kas));
B_abs = textread('B_abs.txt');
B_abs = B_abs(1:length(kas));
C_real = textread('C_real.txt');
C_real = C_real(1:length(kas));
C_imag = textread('C_imag.txt');
C_imag = C_imag(1:length(kas));
C_abs = textread('C_abs.txt');
C_abs = C_abs(1:length(kas));
D_real = textread('D_real.txt');
D_real = D_real(1:length(kas));
D_imag = textread('D_imag.txt');
D_imag = D_imag(1:length(kas));
D_abs = textread('D_abs.txt');
D_abs = D_abs(1:length(kas));
