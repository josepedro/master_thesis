...
\begin{equation}
  r = \Bigg|\frac{\sum_{n=1}^{N} (x_{n} - \overline{x})(y_{n} - \overline{y})}{\sqrt{\sum_{n=1}^{N} (x_{n} - \overline{x})^{2}} \sqrt{\sum_{n=1}^{N} (y_{n} - \overline{y})^{2}}}\Bigg| \times 100,
\end{equation}
...
Em vista da literatura vigente, fenômenos acústicos que envolvem desprendimento de vórtices são muito peculiares pelo fato do campo acústico ser alterado pela transferência de energia cinética vorticial para energia acústica. Para investigar tal fenomenologia é preciso de uma ferramenta matemática capaz de correlacionar o comportamento vorticial do fluido com o campo acústico por ele produzido e para isso o colorário de Howe se torna uma opção promissora.

De acordo com o estudo de \citeonline{howe1984absorption} o colorário de Howe é uma descrição formal da transferência da energia cinética vorticial para o campo acústico num contexto de baixos números de Mach ($M << 1$) e escoamentos isoentrópicos. Tal formulação é expressa por 

\begin{equation}
  <P> = -\rho_{0}\int_{V}< \boldsymbol{\xi} > dV,
\end{equation}
tal que $<P>$\simbolo{$<P>$}{Potência acústica} é a potência acústica dados um período de tempo e um volume $V$, $\rho_{0}$ é a densidade média do fluido e $\boldsymbol{\xi}$\simbolo{$\boldsymbol{\xi}$}{Energia acústica} é a energia acústica instantânea gerada a partir da energia cinética vorticial. A energia acústica se dá na forma 
\begin{equation}
  \boldsymbol{\xi} = \textbf{u}_{ac} \cdot ( \boldsymbol{\omega} \times \textbf{u}),
\end{equation}
tal que $\textbf{u}_{ac}$\simbolo{$\textbf{u}_{ac}$}{Velocidade de partícula} é a velocidade de partícula, $\boldsymbol{\omega}$\simbolo{$\boldsymbol{\omega}$}{Rotacional do fluido} é o rotacional do fluido e $\textbf{u}$ é a velocidade do fluido. Vale ressaltar que a energia acústica $\xi$ num período de oscilação da onda oferece uma análise do surgimento e dissipação da mesma no tempo e no espaço dentro do duto. Para o uso do colorário de Howe foi implementada uma classe dentro do núcleo do \textit{software} Palabos, sendo ativada e instanciada no final de cada simulação do modelo numérico.     